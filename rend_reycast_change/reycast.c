...
static int	hit_wall(t_cfg *cfg, int mx, int my)
{
	if (mx < 0 || my < 0 || my >= cfg->map.h || mx >= cfg->map.w)
		return (1);
	return (cfg->map.grid[my][mx] == '1');
}
...
double	raycast(t_cfg *cfg, int col, int *line_h)
{
	...
	ray_dir = cfg->pl.dir + camera_x * 0.523598; /* 60° FOV = π/3 = 0.523598 */
	delta_x = fabs(1 / cos(ray_dir));
	delta_y = fabs(1 / sin(ray_dir));

	/* 2. стартовая клетка на карте */
	map_x = (int)cfg->pl.x;
	map_y = (int)cfg->pl.y;
	/* 3. вычисляем step и начальные side_dist */
	if (cos(ray_dir) < 0)
	{
		step_x = -1;
	...
	else
	{
		...
		/* 4. шагаем DDA, пока не встретим стену */
	while (1)
	{
	...
	/* 5. «перпендикулярная» дистанция для устранения fish-eye */
	if (side == 0)
		perp = (side_dist_x - delta_x);
	else
		perp = (side_dist_y - delta_y);
		/* 6. высота столба = k / dist */
